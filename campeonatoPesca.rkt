...
(define (inserirCampeonato conn nome n_competidores precoKG)
  (define stmt
    (prepare conn "INSERT INTO campeonato (nome, n_competidores, precoKG) VALUES (?, ?, ?)"))
  (query-exec conn stmt nome n_competidores precoKG))
...
(define (excluirCompetidor conn id_competidor)
  (define stmt
    (prepare conn "DELETE FROM competidor WHERE id_competidor = ?"))
  (query-exec conn stmt id_competidor))

(define (consultarCompetidores conn id_campeonato)
  (define stmt
    (prepare conn "SELECT * FROM competidor WHERE id_campeonato = ?"))
  (define result (query-rows conn stmt id_campeonato))
  (if (not (empty? result))
      (car result)
      #f))
...
(define (atualizarCompetidor conn id_competidor nome cpf idade cidade)
  (define stmt
    (prepare conn "UPDATE competidor SET nome = ?, CPF = ?, ano_nasc = ?, cidade = ? WHERE id_competidor = ?"))
  (query-exec conn stmt nome cpf idade cidade id_competidor))
...
(define (campeonatoPescaIFC-init
         ...
         (btSalvarCampeonato-callback
          (lambda (button control-event)
            ...
            (define n_competidores (send txtNumeroCompetidores get-value))
            (define preco_peixe (send txtPrecoPeixe get-value))
            ...
            (if resultado
                ...
                (begin
                  ...
                  (send dlogResultadoConsulta show #t)
                  (send txtNumeroCompetidores enable #t)
                  (send txtPrecoPeixe enable #t)
                  (set! resultado (inserirCampeonato conn nome n_competidores preco_peixe))
                  ))))
         ...
         (btAtualizar-callback
          (lambda (button control-event)
            ...
            (define cidade (send txtCidade get-value))
            (define idade (send txtIdade get-value))
            (define resultado (atualizarCompetidor conn nome cpf cidade idade))
            (if resultado
                (begin
                  (send msgCompetidor set-label (string-append "O Competidor " nome " foi atualizado com sucesso!"))
                  (send dlogCompetidor show #t)
                  )
                (begin
                  (send msgCompetidor set-label (string-append "O Competidor " nome " não foi atualizado com sucesso!"))
                  (send dlogCompetidor show #t))
                )
            )
          )
         ...
         (lboxCompetidores-choices
          (let ([resultado (consultarCompetidores conn id_campeonato)])
            (if resultado
                (begin
                  (for-each
                   (lambda (row)
                     (send lboxCompetidores insert row))
                   resultado))
                (begin
                  (send msgCompetidor set-label "Nenhum competidor encontrado!")
                  (send dlogCompetidor show #t)))))
